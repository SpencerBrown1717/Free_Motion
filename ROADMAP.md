...
Past work (shipped, Pi lock release):

16. ~~**`v0.2.0` — Pi-first lockdown release.** Tag and publish the locked Pi contract milestone. Release notes in [`docs/releases/v0.2.0.md`](docs/releases/v0.2.0.md): what is locked (protocol v0, eight-command surface, Pi hardware path, model path, env-var contract, twelve-guarantee safety contract, `/status` shape, failure model, benchmark protocol, M5 acceptance gate); what is still open (Jetson Phase 1 next; ESP32 / Arduino deferred; auth / rate limits / multi-device / second transport explicitly out of scope). Tag: [`v0.2.0`](https://github.com/SpencerBrown1717/Free_Motion/releases/tag/v0.2.0). Backward-compat: protocol unchanged from `v0.1.0-alpha` (`v: 0`); every addition opt-in via config flag; default everywhere stays "mock".~~

Next, in priority order — **M5 Jetson Nano port** (same contract, different hardware):

17. **M5 Phase 1 — Jetson Nano port.** Same contract, different hardware. The bring-up plan is locked at [`docs/jetson-phase1.md`](docs/jetson-phase1.md) (Step 8 — must-keep / allowed-to-differ / first target demo / acceptance gate); the dependency, env-var, camera-path, model-runtime, and unsupported-features mapping is at [`docs/jetson-mapping.md`](docs/jetson-mapping.md) (Step 9). The "must remain identical" surfaces (protocol, command set, world state shape, mission decision shape, safety semantics, status semantics, `/stop` ordering, failure model) are listed in [`docs/pi-reference.md`](docs/pi-reference.md) §10 and re-asserted in [`docs/jetson-phase1.md`](docs/jetson-phase1.md) §2. Acceptance is `examples/jetson_closed_loop_demo/` running the canonical command set against real Jetson hardware while every contract in [`docs/pi-reference.md`](docs/pi-reference.md) §6 holds **and** the Jetson rig produces a `pi_follow_bench`-shaped artifact (renamed `jetson_follow_bench` allowed; the schema, sequence, and criteria are not — see [ADR-0013](docs/decisions.md), [`docs/jetson-phase1.md`](docs/jetson-phase1.md) §6). Phase 1 is gated on real Jetson hardware availability; deliverables 1–8 in [`docs/jetson-phase1.md`](docs/jetson-phase1.md) §5 (docs, adapters, factory, tests, CI smoke) can land on a feature branch and merge after gate-4 verification on real hardware.
18. **M5 Phase 2 — ESP32.** Sensor / actuator coprocessor pattern over UART/SPI to a heavier host. Constraint set defined when this ships.
19. **M5 Phase 3 — Arduino.** Simple actuators, low-level timing. Constraint set defined when this ships.
20. **Rate limits, watchdogs, link-loss fail-safe** (Safety module continued). Bench rig is the test bed; bumped from M4 to keep the milestone narrow.
