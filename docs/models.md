...
[`examples/pi_closed_loop_demo/`](../examples/pi_closed_loop_demo/) is the **end-to-end real-hardware version** of that loop (Step 2; the canonical Pi reference path locked in Step 4). It composes `PiCameraSource` + `YoloVision` + `WorldState` + `GemmaMissionControl` + `SafetyGate` + `PiHardwareController` + `MissionLoop` behind the existing `Agent`/`Router`. The loop is allowed to dispatch only `MOVE` (per [ADR-0010](decisions.md)) so an LLM hallucination cannot arm or disarm the device; ARM/DISARM/STOP stay operator-driven through Telegram. Architectural reference: [`docs/pi-closed-loop.md`](pi-closed-loop.md). Locked contract (commands, env vars, safety, status, failure model, M5 port target): [`docs/pi-reference.md`](pi-reference.md).
