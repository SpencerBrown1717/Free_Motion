...
| Closed loop | `MissionLoop` (Step 2) + Step 3 hardening — background `capture → infer → world → plan → MOVE` with fail-isolated stages, stale-world refusal, per-stage degraded summary, hung-tick handling | [`MissionLoop`](freemotion/agent/mission_loop.py), [ADR-0010](docs/decisions.md), [ADR-0011](docs/decisions.md) |
...
- **Not started:** Jetson / ESP32 / Arduino support (M5). Gated on Steps 4–5 above being complete.
